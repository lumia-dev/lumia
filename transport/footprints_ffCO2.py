...
class FootprintFile:
    ...
    def run(self, obslist, emis, step, atmdel=None): 
        ...
        # footprint_empty = [self.getFootprint(obs.obsid, origin=emis.origin) == 0 for iobs, obs in obslist.iterrows()]
        # obslist = obslist.loc[~array(footprint_empty)].copy()
        if step == 'forward' :
...
class FootprintTransport:
    ...
    def runForward(self, tracers=None):
        ...
        # self.obs.observations.dropna(subset=['mix_co2_fossil'], inplace=True) # TODO: this is a temporary fix for the fossil fuel category
    ...
    def runAdjoint(self):
        ...
        for tr in obs_tracers:
            tracers[tr] = []
            for cat in self.rcf.get(f'emissions.{tr}.categories') if isinstance(self.rcf.get(f'emissions.{tr}.categories'), list) else [self.rcf.get(f'emissions.{tr}.categories')]:
