...
def main():
    ...
    # https://stackoverflow.com/questions/49781573/how-to-pass-global-debug-flag-variable-throughout-my-code-should-i-use-argparse
    ymlFile=None
    if(args.rcf is None):
        if(args.ymf is None):
    ...
    # Now that we know where output is written to and that we have a unique identifier for this run, we move the log files to there
    logger.remove()
    logger.add(sys.stderr, level=log_level, format=log_format, colorize=True, backtrace=True, diagnose=True)
    ...
    if args.optimize or args.adjtest or args.gradtest :
        ...
        if args.optimize :
            ...
            sCmd=f"mv {sTmpPrfx}emissions.nc  {sOutputPrfx}emissionsTmp.nc"
            logger.debug(sCmd)
            hk.runSysCmd(sCmd)
            sCmd=f"mv {sTmpPrfx}emissions_apos.nc  {sOutputPrfx}emissions_apos.nc"
            logger.debug(sCmd)
            hk.runSysCmd(sCmd)
            sCmd=f"mv {os.path.dirname(sTmpPrfx)}/congrad_debug.out  {sOutputPrfx}congrad_debug.out"
            logger.debug(sCmd)
            hk.runSysCmd(sCmd)
            tmpDir=os.path.dirname(sTmpPrfx)    
            if(os.path.isfile(tmpDir+'multitracer-run.log')):
                sCmd=f"mv {tmpDir}/multitracer-run.log  {sOutputPrfx}multitracer-run.log"
                logger.debug(sCmd)
                hk.runSysCmd(sCmd)
            if(os.path.isfile(tmpDir+'multitracer-errors-only.log')):
                sCmd=f"mv {tmpDir}/multitracer-errors-only.log  {sOutputPrfx}multitracer-run.log"
                logger.debug(sCmd)
                hk.runSysCmd(sCmd)
    ...
    print("Done. LumiaDA completed its run.")
