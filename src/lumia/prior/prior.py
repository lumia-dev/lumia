...
def _normalize_backend(s: str | None) -> str:
    if not s:
        return "evd"
    s = s.strip().lower()
    return "cholmod" if s in ("cholmod","ichol","sparse","sksparse") else "evd"
...
class PriorConstraints:
    ...
    def setup(cls, dconf: Dict | DictConfig, mapping: Mapping) -> "PriorConstraints":
        # ---- NEW: read backend choice + options
        backend = _normalize_backend(
            getenv("LUMIA_CORR_BACKEND") or dconf.get("run", {}).get("prior", {}).get("correlation_backend", "evd")
        )
        cholmod_opts = dconf.get("run", {}).get("prior", {}).get("cholmod", {}) if backend == "cholmod" else {}
        ...
        for cat in mapping.optimized_categories:
            ...
            # Coarsen to optimization space:
            errvec = mapping.coarsen_cat(cat, data=errmap.data, value_field='prior_uncertainty')

            # ---- pass backend + opts into both correlation builders
            ...
            # Total uncertainty (works with dense or sparse B):
            errtot = calc_total_uncertainty(
                errvec, corr_t[cat].B, corr_h[cat].B, cat.unit_optim, cat.total_uncertainty.units
            )
            logger.info(f"Original uncertainty for category {cat.name}: {errtot:.3f} {cat.total_uncertainty.units}")

            # Scale to target (per year):
            nsec = errvec.loc[:, ['itime', 'dt']].drop_duplicates().dt.sum().total_seconds()
            scalef = (cat.total_uncertainty.m / errtot) * (nsec / (365.25 * 86400))

            # Keep dtype stable (avoids pandas FutureWarning):
            errvec.loc[:, 'prior_uncertainty'] = (errvec['prior_uncertainty'].astype('float32', copy=False) * scalef)

            logger.info(
                f"Uncertainty for category {cat.name} set to {cat.total_uncertainty.magnitude} "
                f"{cat.total_uncertainty.units} (standard deviations scaled by {scalef = })"
            )
