...
class Cluster:
    ...
    def rank(self) -> float:
        if self.size == 1:
            return -1.0
        # use masked sum (cast to float32 to avoid upcasting)
        return abs((self.data * self.mask).sum(dtype=float))

    def transpose(self) -> None:
        self.data = self.data.transpose()
        self.ind = self.ind.transpose()
        self.mask = self.mask.transpose()
        # IMPORTANT: update dy to the current width
        self.dy = self.nx
    ...
    def crop(self) -> "Cluster":
        """Crop edge rows/cols that are completely masked (in place)."""
        mask_r = (self.mask.sum(0) > 0)
        mask_c = (self.mask.sum(1) > 0)
        mask_r = cumsum(mask_r, dtype=bool) * cumsum(mask_r[::-1], dtype=bool)[::-1]
        mask_c = cumsum(mask_c, dtype=bool) * cumsum(mask_c[::-1], dtype=bool)[::-1]

        self.data = self.data[mask_c, :][:, mask_r]
        self.ind  = self.ind[mask_c,  :][:, mask_r]
        self.mask = self.mask[mask_c, :][:, mask_r]
        # keep dy consistent with new width
        self.dy = self.nx
        return self
...
def clusterize(field, nmax: int, mask: NDArray = None, minxsize: int = 1, minysize: int = 1, cat: str = '', indices: NDArray = None):
    ...
    clusters_final: List[Cluster] = []

    nclmax = min(nmax, int((clusters[0].mask > 0).sum()))

    with tqdm(total=nclmax, desc=f"Spatial aggregation of category {cat}") as pbar:
        ncl = len(clusters) + len(clusters_final)
        while ncl < nclmax:
            # pick cluster with max rank
            ind = max(range(len(clusters)), key=lambda i: clusters[i].rank)
            new_c1, new_c2 = clusters[ind].split()
            clusters.pop(ind)

            for cl in (new_c1, new_c2):
                if not cl.mask.any():
                    continue
            ...
            inc = len(clusters) + len(clusters_final) - ncl
            pbar.update(inc)
            ncl += inc

    logger.info(f"Found {len(clusters + clusters_final)} clusters for category {cat}.")
